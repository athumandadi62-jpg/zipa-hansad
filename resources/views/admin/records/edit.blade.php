...
<div class="max-w-4xl">
    ...
    <form action="{{ route('admin.records.update', $record) }}" method="POST" class="space-y-8 bg-slate-800/50 p-8 rounded-2xl border border-slate-700 shadow-2xl">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <select name="type" id="type" onchange="toggleFields()" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    <option value="qa" {{ $record->type == 'qa' ? 'selected' : '' }}>Question & Answer</option>
                    <option value="speech" {{ $record->type == 'speech' ? 'selected' : '' }}>Speech (Hotuba)</option>
                    <option value="motion" {{ $record->type == 'motion' ? 'selected' : '' }}>Motion (Hoja)</option>
                </select>
            ...
            <div class="space-y-2">
                ...
                <select name="meeting_id" id="meeting_id" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    @foreach($meetings as $meeting)
                        <option value="{{ $meeting->id }}" {{ $record->meeting_id == $meeting->id ? 'selected' : '' }}>
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="title" value="{{ $record->title }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2">
                ...
                <select name="primary_participant_id" id="primary_participant_id" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    @foreach($participants as $participant)
                        <option value="{{ $participant->id }}" {{ $record->primary_participant_id == $participant->id ? 'selected' : '' }}>
            ...
            <div class="space-y-2" id="secondary_participant_div">
                ...
                <select name="secondary_participant_id" id="secondary_participant_id" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
                    ...
                    @foreach($participants as $participant)
                        <option value="{{ $participant->id }}" {{ $record->secondary_participant_id == $participant->id ? 'selected' : '' }}>
                            {{ $participant->name }} ({{ $participant->title }})
                        </option>
                    @endforeach
                </select>
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="subject" value="{{ $record->subject }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
        ...
        <div class="space-y-2">
            ...
            <textarea name="content" id="content" rows="6" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">{{ $record->content }}</textarea>
        ...
        <div class="space-y-2" id="secondary_content_div">
            ...
            <textarea name="secondary_content" id="secondary_content" rows="6" class="w-full bg-slate-900 border border-emerald-900/30 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-emerald-500 outline-none transition-all">{{ $record->secondary_content }}</textarea>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-2" id="record_number_div">
                ...
                <input type="text" name="record_number" value="{{ $record->record_number }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
            </div>
            <div class="space-y-2" id="tags_div">
                ...
                <input type="text" name="tags" value="{{ $record->tags }}" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all">
            </div>
        </div>
