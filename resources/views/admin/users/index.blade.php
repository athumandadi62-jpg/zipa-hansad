...
<div class="mb-8 flex justify-between items-center">
    <div class="flex items-center space-x-4">
        ...
        <span class="px-3 py-1 bg-slate-800 text-slate-400 text-xs rounded-full border border-slate-700"> Total: {{ $users->total() }} </span>
    </div>
    <a href="{{ route('admin.users.create') }}" class="flex items-center space-x-2 bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-xl transition-all shadow-lg shadow-indigo-500/20">
    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y divide-slate-700/50">
            ...
            <tr class="hover:bg-slate-700/30 transition-colors group">
                ...
                <td class="px-6 py-4">
                    <span class="px-3 py-1 text-xs font-bold rounded-full {{ $user->role === 'admin' ? 'bg-indigo-500/10 text-indigo-400 border border-indigo-500/20' : 'bg-slate-700 text-slate-400 border border-slate-600' }}">
                        {{ ucfirst($user->role) }}
                    </span>
                </td>
                ...
                <td class="px-6 py-4">
                    <p class="text-slate-400 text-sm">{{ $user->phone_number ?? 'No Phone' }}</p>
                </td>
                <td class="px-6 py-4 text-right">
                    <div class="flex justify-end space-x-2 transition-all">
                        <a href="{{ route('admin.users.edit', $user) }}" class="p-2 text-slate-400 hover:text-white hover:bg-slate-700 rounded-lg transition-all">
                            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                            </svg>
                        </a>
                        @if($user->id !== auth()->id())
                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" onsubmit="return confirm('Silencer? This action cannot be undone.');">
                            @csrf
                            @method('DELETE')
