...
<div class="max-w-4xl">
    <div class="mb-8 flex justify-between items-center">
        ...
        <div class="flex gap-3">
            <a href="{{ route('admin.questions.download', $question) }}" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-sm font-semibold transition-colors flex items-center gap-2">
            ...
            <a href="{{ route('admin.questions.edit', $question) }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-sm font-semibold transition-colors">
                Edit Record
            </a>
        </div>
    </div>
    ...
    <div class="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl">
        ...
        <div class="p-8 border-b border-slate-700 bg-slate-900/30">
            <div class="flex flex-wrap items-center gap-3 mb-4">
                <span class="px-3 py-1 bg-indigo-500/10 text-indigo-400 text-xs font-bold rounded-full uppercase tracking-widest">
                    {{ $question->meeting->name }}
                </span>
                <span class="px-3 py-1 bg-slate-700 text-slate-300 text-xs font-medium rounded-full">
                    {{ $question->meeting->session_number }}
                </span>
                <span class="text-sm text-slate-500">
                    {{ \Carbon\Carbon::parse($question->meeting->date)->format('F d, Y') }}
                </span>
            </div>
            <h1 class="text-3xl font-bold text-white mb-2">{{ $question->subject }}</h1>
            <p class="text-slate-400">Question Record #{{ $question->question_number ?? 'N/A' }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-slate-700">
            <div class="p-8">
                ...
                <h4 class="text-lg font-bold text-white">{{ $question->questioner->name }}</h4>
                <p class="text-slate-400 text-sm">{{ $question->questioner->title }}</p>
            </div>
            <div class="p-8">
                <p class="text-xs font-bold text-emerald-400 uppercase tracking-widest mb-3">Respondent</p>
                <h4 class="text-lg font-bold text-white">{{ $question->respondent->name }}</h4>
                <p class="text-slate-400 text-sm">{{ $question->respondent->title }}</p>
            </div>
        </div>
