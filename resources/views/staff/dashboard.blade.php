...
<div class="mb-12 text-center max-w-2xl mx-auto">
    ...
    <p class="text-slate-400">Welcome back, {{ auth()->user()->full_name }}. Access legislative records and search for information across the HANSAD repository.</p>
</div>
    ...
    <div class="bg-gradient-to-r from-indigo-900/40 to-slate-800/40 border border-indigo-500/30 rounded-3xl p-8 backdrop-blur-sm relative overflow-hidden group">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 relative">
            <div class="flex-1">
                ...
                <h2 class="text-2xl font-bold text-white mb-2">{{ $nextMeeting->name }}</h2>
                <div class="flex flex-wrap items-center gap-y-2 gap-x-6 text-slate-400 text-sm">
                    <div class="flex items-center gap-2">
                        ...
                        {{ \Carbon\Carbon::parse($nextMeeting->date)->format('F d, Y') }}
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {{ $nextMeeting->location ?? 'To be announced' }}
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="w-4 h-4 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
                        </svg>
                        Session #{{ $nextMeeting->session_number }}
                    </div>
                </div>
            </div>
            <div class="flex items-center">
                <a href="{{ route('search.index', ['meeting_id' => $nextMeeting->id]) }}" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-2xl font-bold transition-all hover:scale-105 active:scale-95 shadow-lg shadow-indigo-600/25">
                    Search Records
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
    <!-- Search Action -->
    <a href="{{ route('search.index') }}" class="group bg-slate-800/50 p-8 rounded-3xl border border-slate-700 hover:border-indigo-500 transition-all hover:translate-y-[-4px]">
    ...
    <!-- Records Access -->
    <a href="{{ route('search.index') }}" class="group bg-slate-800/50 p-8 rounded-3xl border border-slate-700 hover:border-emerald-500 transition-all hover:translate-y-[-4px]">
